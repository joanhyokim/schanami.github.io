...
var aolProvider = food.addBidProvider({
     name: 'aol',
     libUri: ' ',
...
aolProvider.setParam('bidRequestConfig', {
         ...
         sizeId: 225,
         roundingConfig: [
         	{from: 0, to: 500, roundFunction: 'fiveCentsRound'},
         	{from: 500, to: -1, roundValue: 500 }],
         adjustment: -20
 });
...
aolProvider.requestBids = function(slots, pushBid, done) {
 ...
 for (var index in slots) {
     ...
     // function factory
     aolProviderParams.params.callback = 'aolAdtechCallback_' + food.util.newId();
     window[aolProviderParams.params.callback] = aolProvider.bidHandlerFactory(slot);

     var bidRequest = food.util.mergeToObject({}, aolProviderParams);     food.util.mergeToObject(bidRequest, slot.getParam('aol'));
...
food.setAuctionProvider({
    ...
    init: function(targeting, done) {
     ...
     //      		if (lazyload == "false") {
					...
					// // food.observe('AUCTION_POST_RUN', function() {
					// // alert(slot.elementId + "/ not" + lazyload);       
					// // alert(slot.name);   
					// // console.log("loggging");
					// // console.log(ami.mensfitness.ads.slots);
					// // console.log("---");	
					// googletag.display(slot.elementId);
					// delete ami.mensfitness.ads.slots[slot.elementId];			
     ...
     //           	}
		...
		if (document.documentElement.clientWidth < 768) {
		    googletag.cmd.push(function() {
		    	googletag.defineOutOfPageSlot('/4216/mob.mensfitness' + adUnit, 'dfp-ad-mobile_interstitial').setTargeting('pos', ['mobile_interstitial']).addService(googletag.pubads());
		...
		googletag.cmd.push(function() {
            ...
            for (var i = 0; i < targeting.length; i++) {			
            	...
            	var lazyload = food.getSlot(tgtObject.elementId).getParam("lazyload");
          		...
          		if (lazyload == "false") {
          			// alert("not lazy: "+tgtObject.elementId);
					googletag.display(tgtObject.elementId);
					googletag.pubads().refresh([ami.mensfitness.ads.slots[tgtObject.elementId]]);
					delete ami.mensfitness.ads.slots[tgtObject.elementId];			
               	}
             }
		});
...
//         // googletag.pubads().refresh([ami.mensfitness.ads.slots["dfp-ad-top_728x90"]]);
//         // delete ami.mensfitness.ads.slots["dfp-ad-top_728x90"];
//         googletag.display('dfp-ad-right1_300x250');
//         // delete ami.mensfitness.ads.slots["dfp-ad-right1_300x250"];
